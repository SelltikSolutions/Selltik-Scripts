...
# Description: Day-2 Integration Patcher & Healer (Rev 11 - The Silencer).
...
# ------------------------------------------------------------------------------
# 5. AI Workflow Injection & Pre-Seeding
# ------------------------------------------------------------------------------
...
if docker exec -u abc Code-Server test ! -d "/config/workspace/${REPO_NAME}"; then
    ...
    if docker exec -u abc Code-Server git clone "$CLONE_URL" "/config/workspace/${REPO_NAME}" > /dev/null 2>&1; then
         ...
         # 7. Add .aider* to .gitignore to prevent nags
         docker exec -u abc Code-Server sh -c "cd /config/workspace/${REPO_NAME} && echo '.aider*' >> .gitignore && git add .gitignore && git commit -m 'Silence Aider' && git push" >/dev/null 2>&1
         log_succ "Aider gitignore applied."
    else
