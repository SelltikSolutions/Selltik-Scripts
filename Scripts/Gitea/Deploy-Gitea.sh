...
#  Version: v10.24-AEGIS-PRIME
# ==============================================================================
#  Architecture: Single-Node Unified Ingress, VPN, & Identity Topology
#  Aegis Prime Fixes:
#  - CAP-01: Restored s6-overlay capabilities (CHOWN, SETUID, SETGID) to WireGuard
#            to prevent init system death spirals during privilege drop.
#  - DB-01: Enforced UID 70 ownership on the Postgres volume, permanently
#           eliminating the STIG-violating DAC_OVERRIDE kernel capability.
#  Void Forged Fixes:
...
services:
  ...
  auth_db:
    ...
    cap_drop: [ALL]
    # DB-01: Removed DAC_OVERRIDE (STIG Violation). Replaced with explicit UID 70 host directory mapping.
    cap_add: [CHOWN, SETUID, SETGID]
    security_opt: [no-new-privileges:true]
    logging: *default-logging
    healthcheck:
  ...
  wireguard_vpn:
    ...
    cap_drop: [ALL]
    # CAP-01: Injected s6-overlay required capabilities to allow the init system to drop its own privileges without panicking.
